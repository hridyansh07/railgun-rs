## the following is a copy of https://extgit.iaik.tugraz.at/krypto/hadeshash/-/blob/master/code/generate_parameters_grain.sage which is used to generate the poseidon constants
...
# Create a directory for the binary blobs if it doesn't exist
os.makedirs("src/bn254/params", exist_ok=True)

def save_to_bin(name, data_list):
    path = f"params/{name}_t{NUM_CELLS}.bin"
    with open(path, "wb") as f:
        for val in data_list:
            # ark-bn254 Fr uses 32-byte little-endian
            f.write(int(val).to_bytes(32, 'little'))
    return path

# Generate the binary files
mds_flat = [item for sublist in linear_layer for item in sublist]
mds_path = save_to_bin("mds", mds_flat)
rc_path = save_to_bin("rc", round_constants)

# Print the Rust file content
print("// Autogenerated file - Binary Backed")
print("use crate::parameters::PoseidonParams;")
print("use ark_serialize::CanonicalDeserialize;")
print("use std::sync::{Arc, OnceLock};")
print()
print("type Scalar = ark_bn254::Fr;")
print()
print(f"static MDS_BIN: &[u8] = include_bytes!(\"{mds_path}\");")
print(f"static RC_BIN: &[u8] = include_bytes!(\"{rc_path}\");")
print()
print(f"pub static POSEIDON_CIRCOM_BN_{NUM_CELLS}_PARAMS: OnceLock<Arc<PoseidonParams<Scalar>>> = OnceLock::new();")
print()
print(f"pub fn get_t{NUM_CELLS}_params() -> Arc<PoseidonParams<Scalar>> {{")
print(f"    POSEIDON_CIRCOM_BN_{NUM_CELLS}_PARAMS.get_or_init(|| {{")
print("        let mds = MDS_BIN.chunks_exact(32)")
print("            .map(|c| Scalar::deserialize_compressed(c).unwrap())")
print(f"            .collect::<Vec<_>>().chunks({NUM_CELLS})")
print("            .map(|c| c.to_vec()).collect();")
print()
print("        let rc = RC_BIN.chunks_exact(32)")
print("            .map(|c| Scalar::deserialize_compressed(c).unwrap())")
print(f"            .collect::<Vec<_>>().chunks({NUM_CELLS})")
print("            .map(|c| c.to_vec()).collect();")
print()
print(f"        Arc::new(PoseidonParams::new({NUM_CELLS}, {alpha}, {R_F_FIXED}, {R_P_FIXED}, mds, rc).unwrap())")
print("    }).clone()")
print("}")
